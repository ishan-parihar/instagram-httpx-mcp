# instagram_mcp_server/logging_config.py
"""
Logging configuration for Instagram MCP Server with format options.
...
from instagram_mcp_server.debug_trace import cleanup_trace_dir, get_trace_dir
...
class CompactFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Shorten the logger name by removing the instagram_mcp_server prefix
        if record_copy.name.startswith("instagram_mcp_server."):
            record_copy.name = record_copy.name[len("instagram_mcp_server.") :]
...
def configure_logging(log_level: str = "WARNING", json_format: bool = False) -> None:
    """Configure logging for the Instagram MCP Server.
