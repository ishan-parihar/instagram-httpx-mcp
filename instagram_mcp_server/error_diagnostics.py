...
from instagram_mcp_server.debug_trace import get_trace_dir, mark_trace_for_retention
...
def _render_issue_template(payload: dict[str, Any]) -> str:
    ...
    reproduction_lines = [
        "1. Run a fresh local `uv run -m instagram_mcp_server --login`.",
...
def _safe_source_profile_dir():
    try:
        ...
        return (Path.home() / ".instagram-mcp" / "profile").expanduser()
...
def _build_gist_command(
    ...
    return f'gh gist create {quoted} -d "Instagram MCP debug artifacts"'
...
def _tool_name_for_context(payload: dict[str, Any]) -> str | None:
    ...
    if context in {"extract_page", "extract_overlay", "scrape_user"}:
        return "get_user_profile"
    if context == "scrape_insights":
