"""
Centralized error handling for Instagram MCP Server using FastMCP ToolError.

Provides raise_tool_error() which maps known Instagram exceptions to user-friendly
...
def raise_tool_error(exception: Exception, context: str = "") -> NoReturn:
    """
    Raise a ToolError for known Instagram exceptions, or re-raise unknown ones.
    ...
    Raises:
        ToolError: For known Instagram exception types
    ...
    if isinstance(exception, CredentialsNotFoundError):
        ...
        raise ToolError(
            "Instagram browser setup was not ready. A fresh setup attempt has started in the background. Retry this tool in a few minutes."
        ) from exception
        ...
        _raise_tool_error_with_diagnostics(
            exception,
            "Element not found. Instagram page structure may have changed.",
            context=context,
        )
        ...
        _raise_tool_error_with_diagnostics(
            exception,
            "Scraping failed. Instagram page structure may have changed.",
            context=context,
        )

    elif isinstance(exception, (InstagramScraperException, InstagramMCPError)):
        ...
        logger.warning("Instagram error%s: %s", ctx, exception)
        _raise_tool_error_with_diagnostics(
            exception,
            str(exception),
            context=context,
