...
from instagram_mcp_server.common_utils import secure_mkdir, slugify_fragment
from instagram_mcp_server.session_state import auth_root_dir, get_source_profile_dir
...
def _trace_mode() -> TraceMode:
    raw = os.getenv("INSTAGRAM_TRACE_MODE", "").strip().lower()
...
def trace_enabled() -> bool:
    return (
        bool(os.getenv("INSTAGRAM_DEBUG_TRACE_DIR", "").strip())
        or _trace_mode() != "off"
    )
...
def get_trace_dir() -> Path | None:
    ...
    explicit = os.getenv("INSTAGRAM_DEBUG_TRACE_DIR", "").strip()
    if explicit:
...
def _safe_source_profile_dir() -> Path:
    try:
        ...
        return Path("~/.instagram-mcp/profile").expanduser()
...
async def record_page_trace(
    ...
    payload = {
        ...
        "instagram_cookie_names": instagram_cookie_names,
