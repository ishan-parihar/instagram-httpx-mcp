...
async def detect_rate_limit(page: Page) -> None:
    """Detect if Instagram has rate-limited or security-challenged the session.
    ...
    if "instagram.com/checkpoint" in current_url or "authwall" in current_url:
    ...
    # Normal Instagram pages (profiles, jobs) have <main> and long content
...
async def scroll_job_sidebar(
    ...
    Instagram renders job search results in a scrollable sidebar container,
