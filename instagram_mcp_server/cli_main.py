...
def clear_profile_and_exit() -> None:
    """Clear Instagram browser profile and exit."""
    config = get_config()
    ...
    version = get_version()
    logger.info(f"Instagram MCP Server v{version} - Profile Clear mode")
    ...
    print(f"🔑 Clear Instagram authentication state from {auth_root}?")
    ...
    if clear_auth_state(get_profile_dir()):
        print("✅ Instagram authentication state cleared successfully!")
    else:
...
def profile_info_and_exit() -> None:
    ...
    version = get_version()
    logger.info(f"Instagram MCP Server v{version} - Session Info mode")
